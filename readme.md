...
|[Фабричный метод](#-фабричный-метод)|[Мост](#-мост)|[Команда](#-команда)|
...
Пример из жизни
> В качестве общего примера можно привести заказ еды в ресторане. Вы (т.е. `Client`) просите официанта (т.е. `Invoker`) принести еду (т.е. `Command`), а официант просто переадресовывает запрос Шеф-повару (т.е. `Receiver`), который знает, что и как готовить.
> Другим примером может быть то, что вы (т.е. `Client`) включаете (т.е. `Command`) телевизор (т.е. `Receiver`) с помощью пульта дистанционного управления (`Invoker`).

Простыми словами
> Шаблон позволяет инкапсулировать действия в объекты. Ключевая идея, лежащая в основе этого шаблона, заключается в том, чтобы предоставить средства для отделения клиента от получателя.

Wikipedia говорит
> В объектно-ориентированном программировании шаблон Команда — это поведенческий шаблон проектирования, в котором объект используется для инкапсуляции всей информации, необходимой для выполнения действия или запуска события в более позднее время. Эта информация включает имя метода, объект, которому принадлежит метод, и значения параметров метода.

**Пример кода**

Прежде всего, у нас есть получатель(Receiver), который выполняет все действия, которые могут быть выполнены
...
затем у нас есть интерфейс, который будет реализовывать каждую из команд, а затем у нас есть набор команд
...
Затем у нас есть вызывающий(Invoker), с которым клиент(Client) будет взаимодействовать для обработки любых команд
...
Шаблон Команда также может быть использован для реализации системы, основанной на транзакциях. Где вы продолжаете вести историю команд сразу после их выполнения. Если последняя команда выполнена успешно, то все хорошо, в противном случае просто просмотрите историю и продолжайте выполнять `undo` для всех выполненных команд.
